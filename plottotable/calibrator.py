...
def getHlabel(img, x, y, w, h): #crop H axis and label data(below x axis)
	print "Cropping Hlabel... ",
	hlabel=img[y:y+h,x:x+w]
	print "Done!"
	return hlabel

def getVlabel(img, x, y, w, h):# crop Y axis and label data(befor y axis)
	print "Cropping Vlabel... ",
	vlabel=img[y:y+h,x:x+w]
	print "Done!"
	return vlabel

def getVerticalSplit(vlabel): #get local minima and split the y axis data image
	print "In getVerticalSplit... ",
	gray = vlabel.copy()
	shape= gray.shape
	rows=shape[0]
	cols=shape[1]
...
def getHorizontalSplit(hlabel): #get local minima and split the x axis data image
	
	print "In getHorizontalSplit... ",
	gray = hlabel.copy()
	shape= gray.shape
	rows=shape[0]
	cols=shape[1]
	...
	for i in range(0,rows-1):
		...
		if begin_check == 1 and percent>99:
			yb=i
			begin_check=0
			if abs(yb-ya)>10: #neglecting small strips of black and noise
...
def imgResize(img, finalDimension): #function to resize image(opencv resizing)
...
def binarize(img, shape): # function to grayscale and threshold binarize image  
...
def preProcessLabel(label,size): #process x and y label data before passig it to the tesseract OCR
	shape=label.shape
...
def rotateYlabel(label): #function to rotate y axis label
	print "Rotating Y label..."
	label= cv2.transpose(label)
	label= cv2.flip(label,1)
	return label

def prepareHocr(img): # function to call tesseract hocr command on an image file
	print "Running OCR on labels... ",
	subprocess.call("tesseract "+img+".jpg outtess hocr digits", shell=True)
	subprocess.call("mv outtess.hocr outtess.html", shell=True)
	print "Done!"
	block=getOCRLines('outtess.html')
	return block

def prepareTesseract(img): # function to run tesseract ocr on image file
	print "In prepareTesseract... ",
...
def getVaxisData(block,rows,size): # function to process hocr block of y axis and return data
	nums=[]
	...
	if len(nums)==2:
		reference=nums[1]
		scale=size/rows
		gradient=abs(float(nums[1][0]-nums[0][0]))/abs(nums[1][1]-nums[0][1])
...
def quantizeAxisEnd(x,step,reference,marker): #function to quantize x axis labels into endpoint values
	rangeQ=float(step/2)
	if marker==1:
		while True:
...
def getXaxisData(block,xo1,xo2,cols,size): # function to get x axis data from hocr block 
	steps=[]
	nums=[]
	nums1=[]
	block=processBlock(block)
	...
	if len(nums) >=2:
		...
		reference=nums1[1]
		gradient=abs(float(nums1[1][0]-nums1[0][0]))/abs(nums1[1][1]-nums1[0][1])

		scale=size/cols
		gradient=gradient*scale
		reference[1]=reference[1]/scale

		minVal=quantizeAxisEnd(reference[0]+gradient*(float(xo1)-reference[1]),step,reference[0],-1)
		maxVal=quantizeAxisEnd(reference[0]+gradient*(float(xo2)-reference[1]),step,reference[0],1)
...
def processBlock(block): # convert hocr block , convert 2d hocr block array to 1d array
	out=[];
	a=""
	for i in block:
		for j in i:
...
def getLabelData(img_path): # Takes image(plot) path and returns all x axis and y axis related data (along with labels)
	print "Fetching label data... ",
	...
	try: #Processing for X axis data
		hlabel= getHlabel(img, 0, imgAxes["haxisY"]+2, cols, rows-imgAxes["haxisY"]-2)
	...
	if not x_error:
		...
		try:
			...
			minVal,maxVal,stepX,gradientX,startPixel=getXaxisData(hocr_block,imgAxes["vaxisX"],imgAxes["vaxisX2"],cols,1000.0)
	...
	if not y_error:
		...
		try:
			...
			gradientY,referenceY=getVaxisData(hocr_block,imgAxes["haxisY"],1000.0)
		except:
			gradientY=False
			referenceY=False
			y_error=True
	...
	if x_error or not xMin or not xMax or not stepX or not gradientX :
		return False, False, False, False, referenceY, gradientY, hlabel_val_text, vlabel_val_text
	else:
		return xMin, xMax, stepX, gradientX, referenceY, gradientY, hlabel_val_text, vlabel_val_text

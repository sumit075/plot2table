...
from legend_detect import fetchLegendColors, getLegendsBox,legendLabelCoordinates
from calibrator import getLabelData
from axisDetector import AxesDetection
from color_kmeans import colorCluster
from preprocessor import sharpenImage, cluster, resizeImage, otsusBinarization, enhance, histogram, getHue
...
def estimateMissingData(Plot_data, legends):
	...
	if Plot_data == None or legends == None:
		print "Either Generate Plot data or Legends is None exiting estimateMissingData()..."
		return 
	...
	for legend in legends:
		...
		for index, val in enumerate(Plot_data[legend["name"]]):
			if not val == "-":
				...
				if not last_val_found["val"] == None:
					next_value = None
					next_index = None
					for next_ind, next_val in enumerate(Plot_data[legend["name"]][index + 1:]):
						# print next_ind, next_val
						if not next_val == "-":
							next_value = next_val
					...
					if not next_value == None:
						...
						Plot_data[legend["name"]][index] = float(next_ratio*prev_value + prev_ratio*next_value)/(prev_ratio+next_ratio)

						last_val_found["val"] = Plot_data[legend["name"]][index]
						last_val_found["index"] = index
					else:
...
def getTables(file_name):
	...
	try:
		...
		print str(e)
		print "Image" + file_name + "not found in multicrop-output, exiting getTables()..."
		return ERROR_CODE
	...
	if len(raw_legends) == 0:
		...
		for clt in cluster_centers:
			...
			if luma <= 240 and luma >= 20:
				...
				# If pixel color is not Gray and pixel is not in legend bounded area
				if (not Gray_flag):
		...
		for i,color in enumerate(legend_hist_clr):
			...
			legend_obj["min_pixel_dist"] = colorDistance([255, 255, 255], [0, 0, 0])
		...
		for legend in raw_legends:
			if not legend[1] is None:
				...
				# Using luma values to find white and black
				luma = 0.2126 *legend_obj["color"][0] + 0.7152 * legend_obj["color"][1] + 0.0722 * legend_obj["color"][2]
	...
	try:
		...
		print str(e)
		print "Image" + file_name + "not found in multicrop-ext-output, exiting getTables()..."
		return ERROR_CODE
	...
	try:
		statrtingPoint, xMAx, xUnit, xGrad, yRef, yGrad, xLabel, yLabel = getLabelData(plot_path)
	except Exception, e:
		print str(e)
	...
	if yGrad == False:
		...
		xLabel = "x-label"
	# If no data found on x and y axis
	if xGrad == False and (not yGrad == False):
	...
	try:
		...
		try: 
			writer.writerow(tuple([xLabel.replace("\n","") + " vs. " + yLabel.replace("\n","")]))
		except:
	...
	for legend in legends:
		try:
			tmp.append(legend["name"].decode('utf-8').encode('utf-8').replace("\"","").replace("#","").replace("\n",""))
		except:
	...
	try:
		...
		print str(e)
		print "Error in tesseract while reading legends to find legends bounding box"
	...
	if not legends[0]["top_left_x"] is None:
		...
		if legend_bounding_box == None:
			print "Legend bounding box not found, creating same as first legend's box..."
		...
		# Get Legends y-range for checking y values in white gaps between legends
		legend_y_coordinates = legendLabelCoordinates(legend_path, [[lbb_tl_x, lbb_tl_y], [lbb_br_x, lbb_br_y]])
		# print lbb_tl_x, lbb_tl_y, lbb_br_x, lbb_br_y
		# print legend_y_coordinates

		if legend_y_coordinates == None:
			print "Legends y-range is not found, creating same as legend bounding box..."
	...
	startingPointX = ZERO
	yRefPoint = yRef[0]
	yRefPixel = yRef[1] - TOP_EXTENDED_PIXELS #taking y ref point for non-extended image
	...
	while xCurrentPixel < img_width:
		...
		while yCurrentPixel < img_height:
			...
			for l,legend in enumerate(legends):
				# if xCurrentPixel==138 and (yCurrentPixel ==361 or yCurrentPixel ==367):# or yCurrentPixel ==229):
				# 	print "pixel value = ", xCurrentPixel, yCurrentPixel, pixel_color
				# 	print "min_dist = ", min_dist
				# 	print legend["name"]," = ",dist,"legend color = ",legend["color"]

				dist = colorDistance(pixel_color, legend["color"])

				if dist == None:
					print "Color Distance is not found, pixel colors are None, skipping legend " + legend["name"] +  "..."
					continue
			...
			if not legends[0]["top_left_x"] is None:
				# Removing background color pixel and black pixels if legend color is not black
				if (xCurrentPixel >= lbb_tl_x and xCurrentPixel <= lbb_br_x):
					for yCoord in legend_y_coordinates:
						...
						if (yCurrentPixel >= y_min and yCurrentPixel <= y_max):
							# don't add this pixel to any of the legend y_pixel_set
							# because it is in legend bounded area
							Flag = False

			# Using luma values to find white and black
			luma = 0.2126 *pixel_color[2] + 0.7152 * pixel_color[1] + 0.0722 * pixel_color[0]
				...
				if (not Gray_flag) and  Flag:
					if len(legends[min_legend_index]["y_pixel_set"]) == 0:
						...
						try:
							...
							hue2 = getHue([legend_c[2], legend_c[1], legend_c[0]])
						except Exception, e:
							print str(e)
							print "Error in getting Hue values, assigning to exclude this pixel..."
							hue1 = 0
							hue2 = 30

						if abs(hue2 - hue1) <= 20:
							legends[min_legend_index]["y_pixel_set"].append(yCurrentPixel)
							legends[min_legend_index]["min_pixel_dist"] = dist
					else:
						dist = colorDistance(pixel_color, legends[min_legend_index]["color"])
						if dist < legends[min_legend_index]["min_pixel_dist"]:
							legends[min_legend_index]["y_pixel_set"] = []
							legends[min_legend_index]["y_pixel_set"].append(yCurrentPixel)
							legends[min_legend_index]["min_pixel_dist"] = dist
						elif dist == legends[min_legend_index]["min_pixel_dist"]:
							legends[min_legend_index]["y_pixel_set"].append(yCurrentPixel)
			# Next iteration
			yCurrentPixel += 1
		
		# for legend in legends:
			# print legend["name"], legend["color"]
			# print xCurrentPixel, "::::::::::::::::::::", legend["name"], ":::::::::::::::::::", legend["y_pixel_set"]
		...
		xCurrentPixel = startingPointX + int(round(itr * (float(xUnit)/10) * (float(1/xGrad))))
		itr += 1
...
def printTables(_file):
	...
	doc = SimpleDocTemplate(pdf_output_dir + base_name + ".pdf", pagesize=A4, rightMargin=30,leftMargin=30, topMargin=30, bottomMargin=18)
	elements = []
	...
	t = Table(data)
	style = TableStyle(style_param + [('SPAN',(1,0),(no_of_columns - 1,0))])
	t.setStyle(style)

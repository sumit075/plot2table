        ...
        if number_of_plots == 0:
            ...
            for f in sorted(os.listdir(multicrop_images_dir)):
                ...
                self.textBrowser.setText(
                    "Processed " + str(file_number) + " of " +
                    str(number_of_plots))
                QtGui.qApp.processEvents()

            if error == -1:
                showdialog(
                    "Could not generate data for some of the extracted plots")
                QtGui.qApp.processEvents()
...
class ExampleApp(QtGui.QMainWindow, gui.Ui_MainWindow):
    ...
    def browse_folder(self):
        self.listWidget.clear()
                              # In case there are any existing elements in the
                              # list
        ...
        if directory:  # if user didn't pick a directory don't continue
            for file_name in os.listdir(directory):
                ...
                self.listWidget.addItem(
                    file_name)  # add file to the listWidget
        ...
        for ff in sorted(os.listdir(path)):
            if ff.endswith('.jpeg') or ff.endswith('.png'):
                ...
                pixmap = pixmap.scaledToWidth(
                    self.pic_hold.frameGeometry().width() - 150,
    ...
    def actOpen(self, form, layout):
        ...
        if not len(fileName) == 0:
            ...
            dialogTextBrowser.connect(
                dialogTextBrowser.openButton, QtCore.SIGNAL('clicked()'),
                partial(dialogTextBrowser.iteratorthis, fileName, form))
...
def convert_pdfs_to_png(_file):
    ...
    call(["pdftoppm", "-png", "-r", "300",
         fold_path + pdf_file, fold_path + name + '/new'])
...
def add_pic_to_thumbnails(form, name, layout):
    ...
    form.connect(label, QtCore.SIGNAL('clicked()'), partial(
        form.add_pic_to_display, form, name))
    pixmap = pixmap.scaledToWidth(
        form.pic_area.frameGeometry().width() - 40,
        QtCore.Qt.SmoothTransformation)
    label.setPixmap(pixmap)
    layout.addWidget(label)
...
def pdf_help(form):
    ...
    for ff in sorted(os.listdir(path)):
        if ff.endswith('.jpeg') or ff.endswith('.png'):
            ...
            pixmap = QtGui.QPixmap(path + '/' + ff)
            pixmap = pixmap.scaledToWidth(
                form.pic_hold.frameGeometry().width() - 150,
                QtCore.Qt.SmoothTransformation)
            label.setPixmap(pixmap)
            label.setAlignment(QtCore.Qt.AlignCenter)
            layout.addWidget(label)
...
def main():
    ...
    try:
        ...
        form.connect(
            form.actionOpen, QtCore.SIGNAL('triggered()'),
            partial(form.actOpen, form, layout))
    except Exception, e:
        print str(e)
    ...
    try:
        ...
        form.connect(
            form.actionPlot, QtCore.SIGNAL('triggered()'),
            partial(plot_data, form, layout))
    except Exception, e:
        print str(e)
    ...
    try:
        ...
        form.connect(
            form.actionSave, QtCore.SIGNAL('triggered()'),
            partial(selected_pdfs, form, layout))
    except Exception, e:
        print str(e)
    ...
    try:
        ...
        form.connect(
            form.actionPrint, QtCore.SIGNAL('triggered()'),
            partial(form.actPrint, form, layout))
    except Exception, e:
        print str(e)
    ...
    try:
        form.connect(
            form.actionusermanual, QtCore.SIGNAL('triggered()'),
            partial(pdf_help, form))
    except Exception, e:
        print str(e)

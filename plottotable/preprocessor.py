...
def makeEnlarged(page, ENLR_THRESH, MC_EXT=150):
	...
	subprocess.call(("bash multicrop -e %d -u 3 " % MC_EXT) + ("raw_%02d.jpg " % page) + ("multicrop-ext-output/new_raw_mc_%02d.ppm " % page), shell=True)
...
def genImages(inPdf, ENLR_THRESH=600, MC_EXT=150):
	...
	for i in range(1, numPages+1):
		...
		subprocess.call("bash multicrop " +  ("-e %d -u 3 " % MC_EXT) + ("output-%d.ppm " % i) + ("multicrop-ext-output/tmp_raw_mc_%02d.ppm " % i), shell=True)
	...
	for im in mc_imgs:
		...
		if isAxisPresent(img):
			...
			try:
				...
				if ax:
					os.remove('multicrop-ext-output/'+im)
					shutil.move('multicrop-output/'+im, 'multicrop-ext-output/'+im)

					# print "================================", ax
					crp_img = crp_img[ax['haxisY2']:ax['haxisY'], ax['vaxisX']:ax['vaxisX2']]
	...
	for i in range(0,len(center)):
		...
		for j in range(0,(img.shape)[0]):
			for k in range(0,(img.shape)[1]):
				if img[j,k,0]==center[i][0] and img[j,k,1]==center[i][1] and img[j,k,2]==center[i][2]:
				 	count_hue+=1
	...
	for i in range(0,len(hue)):
		...
		for j in range(0,len(hue)):
			if hue[i]==hue[j] :
				if maximum<histo[j]:
					max_pos=j
		if (hue[max_pos] in hue_pushed) ==False:

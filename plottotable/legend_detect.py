...
import re
import os
from preprocessor import otsusBinarization, resizeImage, sharpenImage, cluster, enhance, morphological
...
def fetchLegendColors(imgFile):
	...
	print "Running OCR... ",
	subprocess.call("tesseract temp_image.tiff outtess hocr -l eng", shell=True)
		...
		if len(block) == 1:
			...
			if orient is None:
				...
				try:
					box = block[0]
					val = box.split(',')
					x1, y1, x2, y2 = int(val[1]), int(val[2]), int(val[3]), int(val[4])
			...
			if isWord(val[0]):
				...
				# when all ocrwords are words
					...
					try:
						...
						y1, y2 = sorted([int(w_f[2]), int(w_f[4]), int(w_l[2]), int(w_l[4])])[1:3]
						...
						final_crop = np.concatenate((crop_img1, crop_img2), axis=1)
				...
				if isW_flag:
					...
					try:
						...
						x1, y1, x2, y2 = int(w_f[1]), int(w_f[2]), int(w_f[3]), int(w_f[4])

						a1, b1, a2, b2 = int(w_l[1]), int(w_l[2]), int(w_l[3]), int(w_l[4])
